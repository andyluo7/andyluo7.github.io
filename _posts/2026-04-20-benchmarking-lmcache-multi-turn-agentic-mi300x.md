...
We benchmarked multi-turn agentic workloads using 739 anonymized Claude Code conversation traces from [kv-cache-tester](https://github.com/callanjfox/kv-cache-tester) against MiniMax-M2.5 (230 GB FP8 MoE) on 2× AMD MI300X with vLLM 0.19.0 + LMCache (built from source for ROCm). Three KV-cache strategies were compared head-to-head: no cache, vLLM's HBM prefix cache, and LMCache CPU-DRAM offload.
...
1. **Use real workload traces, not synthetic mixes.** The [kv-cache-tester](https://github.com/callanjfox/kv-cache-tester) dataset provides 739 anonymized Claude Code traces. There's no excuse to evaluate L2 caching with toy benchmarks.
...
- **callanjfox / WEKA** for the [kv-cache-tester](https://github.com/callanjfox/kv-cache-tester) toolkit and the 739 anonymized Claude Code agentic traces
